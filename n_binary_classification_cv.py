...
for fold_idx, (train_idx, test_idx) in enumerate(skf.split(X_train, y_train)):
    ...
    cur_feature_holder = X_train_fold
    cur_label_holder = y_train_fold
    ...
    for index, current_class in enumerate(sorted_labels):
        ...
        unique_before, counts_before = np.unique(cur_label_holder, return_counts=True)
        print(f"Current label distribution: {dict(zip(unique_before, counts_before))}")
        
        # Before processing
        current_class_count = (cur_label_holder.flatten() == current_class).sum()
        ...
        train_mask = (cur_label_holder.flatten() == current_class)
        ...
        val_mask = (y_val_fold.flatten() == current_class)
        ...
        val_pred = best_clf.predict(X_val_fold)
        ...
        remaining_current = (cur_label_holder[remaining_mask].flatten() == current_class).sum()
        ...
        cur_label_holder = cur_label_holder[remaining_mask]
    ...
    # For evaluation
    # Evaluate
    curr_ce = weighted_log_loss_test(y_val_fold.flatten(), val_probs)
    cr = classification_report(y_val_fold, np.argmax(val_probs, axis=1), output_dict=True)
    weighted_f1 = cr['weighted avg']['f1-score']
    print(f"Weighted Log Loss: {curr_ce:.8f}")
    print(classification_report(y_val_fold, np.argmax(val_probs, axis=1)))
    folds_ce.append(curr_ce)
    folds_report.append(weighted_f1)
    logger.info(f"Validation scores for fold {fold_idx} - Weighted Cross-Entropy: {curr_ce:.6f}")
    logger.info(f"Validation scores for fold {fold_idx} - Classification Report:\n{cr}")


print(f"Overall validation scores - Weighted Cross-Entropy:")
print(f"Mean: {np.mean(folds_ce):.8f} (std: {np.std(folds_ce):.8f})")
print(f"Overall validation scores - Weighted F1-score:")
print(f"Mean: {np.mean(folds_report):.8f} (std: {np.std(folds_report):.8f})")
logger.info(f"Overall validation scores - Weighted Cross-Entropy: Mean: {np.mean(folds_ce)} (std: {np.std(folds_ce)})")
logger.info(f"Overall validation scores - Weighted F1-score: Mean: {np.mean(folds_report)} (std: {np.std(folds_report)})")
...
# 2025-04-25 08:03:10,644 - INFO - Overall validation scores - Weighted Cross-Entropy: Mean: 0.006427815492912233 (std: 0.00024089097483576428)
# 2025-04-25 08:03:10,652 - INFO - Overall validation scores - Classification Report:
#            class  precision    recall  f1-score    support
# 0              0   0.000000  0.000000  0.000000     18.000
# 1              1   0.200000  0.100000  0.133333      7.000
# 2             10   0.831586  0.508798  0.630791   1081.000
# 3             11   0.443539  0.740833  0.549508     78.000
# 4             12   0.416084  0.222910  0.260383    457.000
# 5             13   0.053830  0.128205  0.074074     62.000
# 6             14   0.079267  0.496880  0.136689    264.000
# 7             15   0.661905  0.606667  0.620513     26.000
# 8             16   0.200000  0.100000  0.133333      6.000
# 9             17   0.567192  0.796740  0.662376    354.000
# 10            18   0.455530  0.483333  0.378167     60.000
# 11            19   0.312759  0.450794  0.334787    177.000
# 12             2   0.000000  0.000000  0.000000      7.000
# 13            20   0.266126  0.660430  0.377728    153.000
# 14            21   0.746210  0.724808  0.733382    269.000
# 15            22   0.000000  0.000000  0.000000      7.000
# 16            23   0.507692  0.583333  0.509841     41.000
# 17            24   0.390637  0.285714  0.283195    385.000
# 18            25   0.253209  0.451201  0.323380    184.000
# 19            26   0.437005  0.495455  0.431618    278.000
# 20            27   0.667810  0.778788  0.668024    107.000
# 21             3   0.223075  0.430769  0.292980     65.000
# 22             4   0.383886  0.517465  0.440131    238.000
# 23             5   0.940500  0.678489  0.781473   4479.000
# 24             6   0.887061  0.833628  0.859022    553.000
# 25             7   0.404440  0.796190  0.534279    103.000
# 26             8   0.752420  0.525280  0.603400    516.000
# 27             9   0.000000  0.000000  0.000000     25.000
# 28      accuracy   0.599600  0.599600  0.599600      2.998
